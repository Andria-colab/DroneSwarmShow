...
def main():
    ...
    try:
        name_targets = get_target_points(image_path, n_drones=N_DRONES, z_height=10.0)
    except Exception as e:
        print(f"Error loading name image: {e}")
        return

    print("Generating Target Formation (Greeting)...")
    greeting_targets = get_text_points("Happy New Year!", n_drones=N_DRONES, z_height=10.0,scale=0.35)
    ...
    # 3. USE CONFIG PARAMETER HERE
    steps = int(TOTAL_TIME / DT)  # Uses 40 from config now
